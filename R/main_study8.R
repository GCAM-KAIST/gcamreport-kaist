...
#'           project = 'labona.dat', qdata = table,
#'           queryname = qn, clobber = FALSE,
#'           saveProj = FALSE, show_col_types = FALSE
#'         )
#'         if (exists("prj")) {
#'           prj <- rgcam::mergeProjects('labonamerged.dat', list(prj, prj_tmp), clobber = FALSE, saveProj = FALSE)
#'         } else {
#'           prj <- prj_tmp
#'         }
#'         rm(prj_tmp)
...
#'
#'     dt_sec1 <- data_query(nonCO2_emiss_sec_query,
#'                           db_path, db_name = 'database_basexdb_2100', prj_name,
#'                           scenario = 'NDC_LTT_test7',
#'                           desired_regions, GCAM_version, queries_nonCO2_file)
#'     dt_sec2 <- data_query(nonCO2_emiss_sec_query,
#'                           db_path, db_name = 'database_basexdb_study8', prj_name,
#'                           scenarios = c('NDC_LTT_AP_USA', 'NDC_LTT_RT_euDom', 'NDC_LTT_AP_IDN', 'NDC_LTT_AP_BRA'),
#'                           desired_regions, GCAM_version, queries_nonCO2_file)
#'     dt_sec <- rbind(dt_sec1, dt_sec2)
#'     prj_tmp <- rgcam::addQueryTable(
...
#'     if (exists("prj_tmp")) rm(prj_tmp)
#'
#'     dt_reg1 <- data_query(nonCO2_emiss_reg_query,
#'                           db_path, db_name = 'database_basexdb_2100', prj_name,
#'                           scenario = 'NDC_LTT_test7',
#'                           desired_regions, GCAM_version, queries_nonCO2_file)
#'     dt_reg2 <- data_query(nonCO2_emiss_reg_query,
#'                           db_path, db_name = 'database_basexdb_study8', prj_name,
#'                           scenarios = c('NDC_LTT_AP_USA', 'NDC_LTT_RT_euDom', 'NDC_LTT_AP_IDN', 'NDC_LTT_AP_BRA'),
#'                           desired_regions, GCAM_version, queries_nonCO2_file)
#'     dt_reg <- rbind(dt_reg1, dt_reg2)
#'     prj_tmp <- rgcam::addQueryTable(
