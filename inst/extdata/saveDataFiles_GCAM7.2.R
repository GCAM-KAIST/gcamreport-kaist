...
reg_cont_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "regions_continents_map.csv"),
                          skip = 1,
...
co2_tech_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "CO2_tech_map.csv"),
                              skip = 1, na = "",
                              stringsAsFactors = FALSE
) %>% gather_map()
use_data(co2_tech_map_v7.2, overwrite = T)

co2_resource_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "CO2_resource_map.csv"),
                              skip = 1, na = "",
                              stringsAsFactors = FALSE
) %>% gather_map()
use_data(co2_resource_map_v7.2, overwrite = T)

kyoto_sector_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "kyotogas_sector.csv"),
                                  skip = 1, na = "",
                                  stringsAsFactors = FALSE
) %>% gather_map()
use_data(kyoto_sector_map_v7.2, overwrite = T)

nonco2_emis_sector_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "nonCO2_emissions_sector_map.csv"),
                                        skip = 1, na = "", stringsAsFactors = FALSE
) %>% gather_map()
use_data(nonco2_emis_sector_map_v7.2, overwrite = T)

nonco2_emis_resource_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "nonCO2_emissions_resource_map.csv"),
                                          skip = 1, na = "", stringsAsFactors = FALSE
) %>% gather_map()
use_data(nonco2_emis_resource_map_v7.2, overwrite = T)

carbon_seq_tech_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "carbon_seq_tech_map.csv"),
                                     skip = 1, na = "",
                                     stringsAsFactors = FALSE
) %>% gather_map()
...
# ag maps
ag_demand_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "ag_demand_map.csv"),
                               skip = 1,
                               stringsAsFactors = FALSE
) %>% gather_map()
use_data(ag_demand_map_v7.2, overwrite = T)

ag_price_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "ag_price_map.csv"),
                               skip = 1,
                               stringsAsFactors = FALSE
) %>% gather_map()
use_data(ag_price_map_v7.2, overwrite = T)

ag_production_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "ag_production_map.csv"),
                               skip = 1,
                               stringsAsFactors = FALSE
) %>% gather_map()
use_data(ag_production_map_v7.2, overwrite = T)

ag_demand_price_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "ag_demand_price_map.csv"),
                               skip = 1,
                               stringsAsFactors = FALSE
)
use_data(ag_demand_price_map_v7.2, overwrite = T)

trade_ag_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "trade_ag.csv"), skip = 1,
                                      stringsAsFactors = FALSE) %>% gather_map()
use_data(trade_ag_v7.2, overwrite = T)

land_use_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "land_use_map.csv"),
                              skip = 1,
                              stringsAsFactors = FALSE
) %>% gather_map()
use_data(land_use_map_v7.2, overwrite = T)

food_intake_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "food_intake_map.csv"),
                              skip = 1,
                              stringsAsFactors = FALSE
) %>% gather_map()
use_data(food_intake_map_v7.2, overwrite = T)

food_items_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "food_items_map.csv"),
                              skip = 1,
                              stringsAsFactors = FALSE
)
...
primary_energy_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "primary_energy_map.csv"),
                                    skip = 1,
                                    stringsAsFactors = FALSE
) %>% gather_map()
use_data(primary_energy_map_v7.2, overwrite = T)

production_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "production_map.csv"),
                                skip = 1,
                                stringsAsFactors = FALSE
) %>% gather_map()
use_data(production_map_v7.2, overwrite = T)

secondary_energy_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "capacity_map.csv"),
                              skip = 1,
                              stringsAsFactors = FALSE
) %>%
...
capacity_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "capacity_map.csv"),
                              skip = 1,
                              stringsAsFactors = FALSE
) %>%
  dplyr::filter(!grepl("cogen", technology)) %>%
...
cf_gcam_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "A23.globaltech_capacity_factor.csv"),
                         skip = 9, na = "",
                         stringsAsFactors = FALSE
)
use_data(cf_gcam_v7.2, overwrite = T)

cf_rgn_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "L223.StubTechCapFactor_elec.csv"),
                        skip = 1, na = "",
                        stringsAsFactors = FALSE
)
use_data(cf_rgn_v7.2, overwrite = T)

final_energy_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "final_energy_map.csv"),
                                  skip = 1,
                                  stringsAsFactors = FALSE
) %>% gather_map()
use_data(final_energy_map_v7.2, overwrite = T)

en_demand_price_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "en_demand_price_map.csv"),
                                  skip = 1,
                                  stringsAsFactors = FALSE
)
...
energy_price_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "en_price_map.csv"),
                                   skip = 1, na = "",
                                   stringsAsFactors = FALSE
) %>% gather_map()
...
# Energy Service maps
transport_en_service_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "transport_en_service.csv"),
                                      skip = 1,
                                      stringsAsFactors = FALSE
) %>% gather_map()
use_data(transport_en_service_v7.2, overwrite = T)

buildings_en_service_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "buildings_en_service.csv"),
                                      skip = 1,
                                      stringsAsFactors = FALSE
) %>% gather_map()
...
# capital updates
capital_gcam_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "L223.GlobalIntTechCapital_elec.csv"),
                              skip = 2, na = "",
                              fileEncoding = "UTF-8-BOM", stringsAsFactors = FALSE
) %>%
  dplyr::rename(technology = intermittent.technology) %>%
  dplyr::bind_rows(read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "L223.GlobalTechCapital_elec.csv"),
                            skip = 2, na = "",
                            stringsAsFactors = FALSE
  )) %>%
...
investment_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "investment.csv"),
                            na = "",
                            fileEncoding = "UTF-8-BOM", stringsAsFactors = FALSE
) %>%
...
carbon_content_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "L202.CarbonCoef.csv"),
                                skip = 2, na = "",
                                stringsAsFactors = FALSE
)
use_data(carbon_content_v7.2, overwrite = T)

nonco2_content_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "L201.ghg_res.csv"),
                                skip = 2, na = "",
                                stringsAsFactors = FALSE
)
use_data(nonco2_content_v7.2, overwrite = T)

iea_capacity_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "IEAWEO2019_Capacity.csv"), stringsAsFactors = FALSE)
use_data(iea_capacity_v7.2, overwrite = T)

co2_market_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "CO2market_new.csv"), skip = 1, stringsAsFactors = FALSE)
use_data(co2_market_v7.2, overwrite = T)

co2_market_frag_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "CO2market_frag_map.csv"),
                                     skip = 1,
                                     stringsAsFactors = FALSE
)
...
# iron and steel
iron_steel_trade_map_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2", "iron_steel_trade.csv"), skip = 1,
                                      stringsAsFactors = FALSE) %>% gather_map()
...
region_mapping_ucd_v7.2 <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.2",
...
# Read in template
template_v7.2 <- readxl::read_excel(file.path(rawDataFolder, "inst/extdata", "template/GCAM7.2/common-definitions-template.xlsx"),
                                 sheet = 'variable'
 ) %>%
